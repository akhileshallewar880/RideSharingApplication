...
namespace RideSharing.API.Services.Notification;
...
public class FCMNotificationService
{
    ...
    {
        _logger.LogInformation($"🔔 SendBookingConfirmationAsync called - IsInitialized: {_isInitialized}, MessagingNull: {_messaging == null}");
        
        if (!_isInitialized || _messaging == null)
        {
            _logger.LogWarning("⚠️ Firebase not initialized. Skipping notification.");
            return;
        }
        
        try
        {
            _logger.LogInformation($"📤 Preparing to send notification to token: {fcmToken.Substring(0, Math.Min(20, fcmToken.Length))}...");
            ...
            _logger.LogInformation($"📨 Sending FCM message...");
            var response = await _messaging.SendAsync(message);
            _logger.LogInformation($"✅ Booking confirmation sent successfully! MessageId: {response}");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, $"❌ Failed to send booking confirmation to token: {fcmToken.Substring(0, Math.Min(20, fcmToken.Length))}...");
    ...
    /// </summary>
    public async Task SendNewBookingToDriverAsync(
        string fcmToken, 
        ...
        try
        {
            var message = new Message
            {
                ...
                Notification = new FcmNotification
                {
                    ...
                    Body = $"{passengerName} booked {passengerCount} seat(s) from {pickupLocation} to {dropoffLocation}"
                },
                Data = new Dictionary<string, string>
                {
                    { "type", "new_booking" },
            ...
            var response = await _messaging.SendAsync(message);
            _logger.LogInformation($"✅ New booking notification sent to driver. MessageId: {response}");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "❌ Failed to send new booking notification to driver");
        }
    }
